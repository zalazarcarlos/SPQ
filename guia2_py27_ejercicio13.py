...
L_mano = 10 + 5 + 7.5  #tramo hasta el manómetro
...
def ejercicio13():
    ...
    for i in range(0,1001,1):
        ...
        def FOsolPm (p_mano_pascal):
            
            F0 = (p1_pascal - p_mano_pascal) / rho \
            - fd * L_mano * v ** 2 * 0.5 / d_m \
            - 2 * (K * (v**2) / 2) \
            - z
            return F0
        
        sol = newton(FOsolPm, 150000)
        p_mano = sol / 100000
        
        P_MANO = np.append(P_MANO, p_mano)
    ...
    valores = {'tiempo_s':T,
               'p1_bar':P1,'pmano_bar':P_MANO, 'p2_bar':P2,
               'velc_m/s':V,
               'F_m3/s':F, 'F_m3/h':F_m3h}
    columnas = ['tiempo_s', 'p1_bar','pmano_bar', 'p2_bar', 'velc_m/s', 'F_m3/s', 'F_m3/h']
