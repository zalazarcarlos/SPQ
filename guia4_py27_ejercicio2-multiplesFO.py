...
#        VP = np.append(VP, vpt)
        ...
        def FOsolv (x):
            ...
            F03 = (pB_pascal - p2_pascal) / (rho*9.8) \
                  - fd * L2/d_m * v**2/(2*9.8)
            
            #F04 = (Kv_SI * vpt * ((pA_pascal -  pB_pascal)/ G)**0.5) - v*area
            
            return [F01, F02, F03] 
        
        semilla = [1.5, 90000, 220000]
    ...
    valores = np.vstack((T,P1, PA_bar, PB_bar, P2, H, V, F_m3h))
    valores = valores.T
    
    columnas = ['tiempo_s', 'p1_bar', 'pA', 'pB', 'p2_bar', 'H_m',
                'vel', 'F_m3/h']
