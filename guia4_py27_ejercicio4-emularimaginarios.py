...
def ejercicio1():
    ...
    for i in range(0,1001,1):
        ...
        v_semilla = np.sqrt(2 * np.sqrt(deltaP_h**2) * d_m/((L1 + Lmenor) * fd))
        ...
        if deltaP_h > 0:
            ...
            def FOsolv (v1):
                ...
                v2 = np.sqrt((2 * deltaP_h + v1**2 * (1-fricc1)) \
                      / (1 + fricc2))
                      
                v3 = np.sqrt((2 * deltaP_h + v1**2 * (1-fricc1)) \
                      / (1 + fricc3))
            ...
            def FOsolv (v1):
                ...
                v2 = -1 * np.sqrt((-2 * deltaP_h + v1**2 * (1-fricc1)) \
                      / (1 + fricc2))
                      
                v3 = -1 * np.sqrt((-2 * deltaP_h + v1**2 * (1-fricc1)) \
                      / (1 + fricc3))
...
#    #creacion de tabla df para pandas:
    valores = np.vstack((T, P1, P2, 
                         V1, V2, V3, F1_m3h, F2_m3h,F3_m3h))
    valores = valores.T
